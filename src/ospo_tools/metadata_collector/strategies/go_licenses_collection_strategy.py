...
class GoLicensesMetadataCollectionStrategy(MetadataCollectionStrategy):
    def __init__(self, repository_url):
        ...
        result = os.system("git clone --depth 1 {} {}".format(quote(repository_url), quote(self.temp_dir_name)))
        if result != 0:
    ...
    def augment_metadata(self, metadata):
        ...
        if not self.go_licenses:
            ...
            for package in metadata:
                if not package.origin and package.name.startswith('go:'):
                    package.origin = self.infer_origin_heuristic(package.name.replace('go:', ''))
        ...
        raise NotImplementedError('GoLicensesMetadataCollectionStrategy.augment_metadata is not implemented')
