    ...
    def augment_metadata(self, metadata):
        ...
        for package in metadata:
            ...
            for sbom_package in packages_in_sbom:
                ...
                old_package_metadata = next((old_package_metadata for old_package_metadata in metadata if old_package_metadata.name == sbom_package['name']), None)
                new_package_metadata = next((new_package_metadata for new_package_metadata in updated_metadata if new_package_metadata.name == sbom_package['name']), None)
                ...
                if not version and 'versionInfo' in sbom_package and sbom_package['versionInfo'] != 'NOASSERTION':
                    version = sbom_package['versionInfo']
                if not origin and 'downloadLocation' in sbom_package and sbom_package['downloadLocation'] != 'NOASSERTION':
                    origin =  sbom_package['downloadLocation']
                if not license and 'licenseDeclared' in sbom_package and sbom_package['licenseDeclared'] != 'NOASSERTION':
