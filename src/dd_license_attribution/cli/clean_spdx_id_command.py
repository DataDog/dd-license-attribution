...
def clean_spdx_id(
    ...
    descriptions to valid SPDX license expressions using LLMs.

    This command reads a CSV file (typically generated by generate-sbom),
    identifies licenses that are long text descriptions instead of SPDX
    license expressions, and uses an LLM to convert them to proper SPDX
    license expressions (including composite licenses like "MIT OR Apache-2.0").
