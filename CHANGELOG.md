...
### Added
- The runtime dependency closure is now pinned in a committed `pylock.toml` (PEP 751) lockfile, and `dd-license-attribution` prefers it when scanning Python projects: dependency enumeration installs the hashed, pinned closure instead of resolving the latest PyPI versions at scan time, making SBOM output (including this repository's own `LICENSE-3rdparty.csv` validation) reproducible. The scan environment's pip is upgraded to a PEP 751 capable version (>= 26.1) before the lockfile install, since fresh virtualenvs may bundle an older pip, and the venv bootstrap's setuptools seed is pruned (via `pip freeze --all`, which skips it by default). For projects pinned by a pylock.toml, the SBOM enumeration comes from the lockfile itself rather than `pip list`, so the venv's bootstrap pip (scan toolchain, not a dependency of the scanned project) no longer leaks into the SBOM and a new pip release can no longer change the generated output - while a pip the scanned project genuinely declares as a runtime dependency stays in the lockfile and in the enumeration - the pip entry's marker is evaluated against the scan virtualenv's own interpreter (its marker environment is collected from the venv's python, via the newly-declared `packaging` dependency - not the interpreter running the scan), so a marker-gated pip entry that does not apply here never reintroduces the toolchain copy. When the lockfile cannot be installed (a platform without matching wheels for a pinned artifact, or an offline pip upgrade), the scan falls back to an unpinned install with a warning - except for lock-only projects (a pylock.toml with no build manifest), which degrade with a warning instead of attempting `pip install .`. Because only the locked closure is installed (the scanned project itself is no longer installed into the enumeration environment), the generated third-party CSV no longer lists the scanned project as a dependency row. Regenerate the lockfile with `uv pip compile --universal --generate-hashes --format pylock.toml pyproject.toml -o pylock.toml` whenever the runtime dependencies in `pyproject.toml` change.
...
- New `package-version` input for the composite GitHub Action. Controls which version of `datadog-license-attribution` is installed from PyPI. Defaults to the pinned release version (`0.7.0`); set to `latest` to install the newest published release. Ignored when `skip-install` is `true`.
