...
- New `package-version` input for the composite GitHub Action. Controls which version of `datadog-license-attribution` is installed from PyPI. Defaults to the pinned release version (`0.7.1`); set to `latest` to install the newest published release. Ignored when `skip-install` is `true`.
